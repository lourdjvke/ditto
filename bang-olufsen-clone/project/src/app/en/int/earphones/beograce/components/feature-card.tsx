...
export default function FeatureCard({ d, cids, styles }: { d: FeatureCardData; cids: string[]; styles: FeatureCardStyles }) {
  return (
    <div data-cid={cids[0]} className={cn("border-t border-solid border-t-surface block overflow-hidden", styles.className)}>
      ...
      <button data-cid={cids[4]} className="w-full flex py-5 text-foreground text-left cursor-pointer max-md:px-4 2xl:hidden" data-component="button" aria-controls={d.ariacontrols2} aria-expanded="false" id={d.id2} type="button">
        <h3 data-cid={cids[5]} className="block flex-1 text-border text-sm leading-6 tracking-[1.7px] uppercase max-lg:text-xs max-lg:leading-5 max-md:tracking-[1.5px] 2xl:hidden" data-component="heading">
          {d.title2}
        </h3>
